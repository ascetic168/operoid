...
**Agent（代理）** 與 **Employee（員工）** 是同一件事。我們使用「員工」這個詞，是因為整個系統是模擬真實組織的運作方式：人們擔任角色、承擔責任、使用工具、產出成果，並且在工作之中持續存在。Operoid 把這個模型帶到 AI 身上。

名字也由此而來：一個會*運轉*的存在 —— 持續運行、持續工作、擔任角色並把事做完 —— 就是一個 **operoid**。平台是 Operoid，而在其中工作的代理就是 operoid。至於本手冊全書使用的正式架構術語，仍維持 **Employee**。
...
本手冊定義 Operoid 的**核心抽象**：系統由哪些物件組成、它們之間的關係，以及貫穿一切的设计哲學。
...
**Part I — 願景**：說明 Operoid *為何* 存在，以及所有決策都必須遵守的原則。
