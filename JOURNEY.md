...
| 2026-08-20 | —（穩健性） | **崩潰復原＋服務模式環境**。① 啟動崩潰復原：孤兒 `Working`/`InProgress` 員工救回可掃描狀態（`fe2545a`；`ocore/runtime.rs`＋store 擴充）。② 服務模式 API key 快照注入——修復 Windows 服務（LocalSystem）看不到使用者環境變數（`60e55a2`）。 | v0.3.0 release | — |
| 2026-08-21 | —（服務/CI） | **Linux/macOS 開機即啟服務**——systemd system unit／launchd LaunchDaemon 以安裝使用者身分執行＋README 三語安裝注意事項（`1f771c6`）；新增 CI check workflow＋service.rs 跨平台編譯修復（`1caf9d6`／`90df7d7`／`119b0b0`）。 | v0.3.0 release | Linux/macOS 服務未實機驗證 |
| 2026-08-25 | **v0.3.1（對齊 gbrain v0.46）** | 對齊 gbrain v0.46 三項：① **MCP provider**——think/query 優先走 `gbrain serve` stdio MCP（rmcp 3.x，新檔 `ocore/gbrain_mcp.rs`），失敗自動 fallback CLI 子行程；管理操作仍走 CLI；新設定 `gbrain_transport`（mcp 預設／cli），設定頁可切。② **search/think 分離**——新增 `GbrainSearchTool`（`gbrain query` 混合檢索，無 LLM 合成、省 token）＋`GbrainToolset` 複合分派器；操作頁新增 query 按鈕；**gbrain v0.46 起 `ask` 已是 `query` 的別名**（純檢索），帶引用的合成回答改由 think 提供。③ **schema pack v2 對齊**——factory frontmatter 加 `origin: operoid-factory`；legacy v1 腦顯示遷移橫幅＋unify-types 按鈕。對話回合 user prompt 與自主循環 PLAN prompt 注入現在時間（日期感知）；同通道單一回覆保證（`bce36f9`）。 | 模型層重整（進行中） | — |
| 2026-08-27～31 | —（模型層＋plane 事實修正） | **模型層重整＋config plane 事實修正**。① 預設模型 `glm-5.3-flash` → **回退 `zhipu:glm-4-flash`**：gbrain（≤0.47.x）對 GLM 未視為 thinking-by-default（zhipu recipe 缺 `thinking_by_default`，上游 issue garrytan/gbrain#4727），glm-5.x 的 think/chat 分別被壓在 4000/4096 output tokens，長回應易 `LLM_OUTPUT_TRUNCATED`。② think 顯式 model 改取 **`models.think`**（缺時 fallback `chat_model`，`ff6ec33`）——thinking 模型的推理 token 計入合成 max_tokens 額度，think 可獨立指到非推理模型；手動操作／agent CLI／MCP 三路徑一致。③ 修 unify-types 按鈕缺 `target_pack`/`apply`/`--follow` 參數（`8fba54c`）。④ **config plane 事實修正（gbrain 0.47.6 實測，`411b273`）**：model/tier 鍵實為 **file plane（config.json）優先、DB 值被 shadow**——與 v0.2.1 的「DB 權威」假設**相反**；`set_model`/`set_models_all`/`sync_new_brain_models` 改**兩 plane 同步寫入**，設定頁 tier 顯示改用 `config get` 有效值＋file/db 來源徽章。⑤ NSIS 反安裝移除 oserver 開機服務，避免孤兒服務（`c65324c`）。 | v0.3.1 release（08-31 發布） | 上游 #4727 修復後可升回 glm-5.x（待處理清單 E15） |
| 2026-09-01 | **v0.3.2（schema v2＋防護）** | ① **工廠頁 gbrain schema v2（15 類型）＋動態 schema pack**——新檔 `ocore/factory_types.rs`（內建 `gbrain-base-v2` 15 型＋legacy 6 型的目錄/frontmatter/管線/同義詞），原散落 5 處的 hardcoded match 全改查表、未知類型明確報錯；pack 取自 gbrain config file plane 的 `schema_pack`；工廠頁改主從式（chip 清單＋單一工作區、高度固定不捲頁）；分類器 pack 感知（`d3ad57e`）。② **think 引註清單**——`gbrain think --json` 重排為人類可讀格式＋「引註（Citations）」清單（雙括號 wikilink 可點擊開筆記）；隱去恆為雜訊的引註比對警告；剔除 `[slug#N]` 佔位（`839af11`）。③ 視窗啟動最大化（避 Windows DPI 置中計算不準）。④ **oserver 舊碼防護**——dev 改 `npm run dev:all`（先編 oserver 再起前端，修「GUI 新、服務舊」路由 404）；NSIS PRE/POSTINSTALL 停止／重啟在跑服務，修「服務模式＋升級安裝」舊碼續跑或覆寫 exe 失敗的縫隙。 | — | `--json` 模式下 think answer 不再逐行串流（已知取捨，顯示「合成中…」提示） |
| 2026-09-07 | **Harness 補強 W0–W3 ✅** | **以 Harness 一般性定義檢視專案後的四 workstream 補強**（計畫 `docs/Operoid-計畫-Harness補強.md`；使用者定調 D-H1 合作式停止／D-H2 產出目錄＋人工晉升／D-H3 建構期 allowlist）。**W0（E8）**：`dispatch_event` 路由命中即 fire-and-forget `brain_sync`。**W1 可攔截性＋E13**：合作式停止（`AppState.cancel` 旗標＋四個步驟邊界檢查點＋`AutonomousOutcome::Cancelled`——承諾維持 Active、員工轉 Paused、`POST /stop`＋watch 停止鈕＋右鍵選單；人類訊息／交辦／核可＝明示恢復）；員工封存（`archived` 欄位、四處過濾、歷史可追溯、已封存檢索區＋解封；`DELETE` 語意改封存、`?hard=true` 串聯硬刪僅 dev/test）。**W2 恢復力（T2）**：LLM 5xx 納入重試＋指數退避、usage 記 `llm` 事件；承諾 `Errored` 自動重排（10min·2^n cap 4h、上限 3 次後 `retry_exhausted` 待人類）、`scan_commitments` 每 tick 只喚醒退避到期者。**W3 行動工具**：`WriteNoteTool`（write-note）——寫 `.md` 到 `employee_output_path`（repo 外不入圖譜；人工 review 後移入 repo 走 sync 晉升＝Ch.07 §6 衍生知識最小落地）；防護（單一檔名／僅 .md／拒覆寫／包含檢查）＋`origin: operoid-employee` 標記；`Employee/Template.tools` allowlist（部署繼承、模板核取）；對話 `write` 動作（Committed artifact）＋PLAN `tool=write` 分支（Draft、Satisfied 晉升）；「開啟產出目錄」入口。`cargo test` ocore **152 全綠**（+19）、workspace 0 warning、npm build 0 error。 | 使用者驗收（tauri dev 視覺＋真實環境觀察） | — |
| 2026-09-18 | **v0.3.4（模型升回 glm-5.3-flash）** | **預設模型升回 glm-5.3-flash——解 E15**。上游 garrytan/gbrain#4727 已於 gbrain 0.50.x 為 zhipu recipe 補 `thinking_by_default`（gbrain 不再將 GLM 5.x 誤判為非 thinking 模型），實測 think/chat 的 model-aware cap 恢復 **16000/32000**（`output_tokens` 精確停在 16000 驗證）；`DEFAULT_CHAT_MODEL` 由 `zhipu:glm-4-flash` 升回 `zhipu:glm-5.3-flash`，常數文檔與測試記錄完整升降級脈絡，三語系「主模型」placeholder 與 BrainsView 新增腦預設值同步（`182684b`）。版號 v0.3.4＋CHANGELOG＋tag 雙推（github／gitea）。 | GitHub Releases draft 審閱後 publish | — |

**目前所在：** **v0.3.4（2026-09-18）**——預設模型升回 glm-5.3-flash（上游 gbrain#4727 已修，解 E15）。此前：v0.3.0 前後端分離、v0.3.1 對齊 gbrain v0.46、v0.3.2 schema pack v2、v0.3.3 Harness 補強（合作式停止＋員工封存＋恢復力＋write-note）。Handbook 五里程碑＋Phase 6/7＋Event 匯流排＋前後端分離皆已完成；開放項見待處理清單（E14／Harness 驗收與版號同步等）。

**（歷史）Phase 7（人機協作）完成——7a 交辦＋7b 聊天＋7c 員工提案核可全數落地。** 人機介面從單發 Q&A 升級為完整協作：雙向多次聊天（Reasoner 驅動回覆、可反問）、交辦承諾（立即喚醒自主跑）、員工在對話中主動提案承諾（Proposed）待人類核可（Active）。Handbook 新增 Message 一級概念（Ch.16，Part IV 重編）＋修訂 Ch.11（Proposed/Rejected）＋Ch.20 §5（提案-核可通用化）。**Phase 6＋7 全部完成**——員工生命週期＋人機協作的完整願景已兌現。
...
**gbrain v0.46 對齊與 schema v2（2026-08-25～09-01，v0.3.1/v0.3.2）**：① MCP provider——think/query 優先走 `gbrain serve` stdio MCP（rmcp 3.x）、失敗 fallback CLI，`gbrain_transport` 可切換；② search/think 分離——`GbrainSearchTool`（query 純檢索、省 token）＋`GbrainToolset` 分派器（v0.46 起 `ask`＝`query` 別名）；③ 模型層重整——預設模型回退 `zhipu:glm-4-flash`（上游 gbrain#4727，追蹤於待處理清單 E15）、think 顯式 model 取 `models.think`；④ **config plane 事實修正**——gbrain 0.47.6 實測 **file plane 優先、DB 被 shadow**（推翻 v0.2.1 的「DB 權威」假設），model 設定改兩 plane 同步寫入（待處理清單 R1/E3/E9 已加更正註記）；⑤ 工廠頁 schema pack v2——15 類型動態查表（`factory_types.rs`）、分類器 pack 感知；⑥ think 引註清單——`--json` 重排＋可點擊 wikilink；⑦ 穩健性——崩潰復原（孤兒狀態救回）、服務模式 API key 快照注入、Linux/macOS 開機服務、NSIS 升級／反安裝的服務生命週期防護。

**Harness 補強完成（2026-09-07，W0–W3，v0.3.3 待發布）**：以 Harness 一般性定義（聚合迴圈／工具層／上下文管理／恢復力／安全）檢視專案後，補齊三層欠缺——可攔截性（合作式停止＋員工封存）、恢復力（LLM 5xx 退避重試＋usage 記錄＋承諾自動重排解 T2）、行動能力（write-note 工具——員工產出寫入 repo 外專屬目錄，人工晉升入圖譜＝Ch.07 衍生知識最小落地；建構期 allowlist 閘門）。兩個行為變更：`DELETE /api/employees/{id}` 預設改為封存、出錯承諾會自動重試（上限 3 次後轉人工）。計畫 `docs/Operoid-計畫-Harness補強.md`。
