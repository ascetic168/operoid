//! Operoid Agent-OS Runtime（P1b 起居於 ocore）——wake→restore→execute→commit→sleep 的執行引擎。
//!
//! 原 src-tauri/runtime.rs 的核心邏輯（run_cycle／run_autonomous／run_inbox／對話回合／
//! Reasoner／Tool），剝離 `#[tauri::command]` 層（留在桌面殼）後搬入。AppHandle 相關的
//! `agent_db_path`／`agent_store` 與 29 個指令薄層見 src-tauri/src/runtime.rs。
...
///
/// 供兩處共用：`run_inbox`（吃完所有 inbox 再睡）與 `run_autonomous`（每輪先清一個——訊息／
/// 事件 review 優先於承諾自主推進，修復 doc/impl 不一致造成的佇列延遲）。
async fn process_one_inbox_task(
    employee_id: &str,
    workspace_id: &str,
    memory: &mut Memory,
    tool: &dyn Tool,
    ...
    let artifact_id = match (task.objective.as_str(), reasoner) {
        ...
        _ => {
            ...
            // 訊息 task 但無 Reasoner → 仍寫 Out Message（gbrain 證據作為回覆），讓對話不斷線。
            if task.objective == "Human message" {
                let _ = store.put_message(&Message {
                    id: fresh_id("msg-out"),
                    workspace_id: workspace_id.to_string(),
                    employee_id: employee_id.to_string(),
    ...
    task.output_artifact_id = artifact_id.clone();
    store.put_task(&task)?;
    ...
    match artifact_id {
        Some(aid) => {
            memory.notes.push(if task.objective == "Human message" {
                format!("replied \"{}\" → {}", task.input, aid)
            } else {
                format!("inbox \"{}\" → artifact {}", task.objective, aid)
            });
            memory.last_artifact_id = Some(aid);
        }
        None => memory
            .notes
            .push(format!("replied \"{}\" (no artifact)", task.input)),
    }
    Ok(true)
}
...
// ───────────────── 對話回合（Phase 7b，Ch.16 Message）─────────────────
/// 時間戳為底的唯一 id（給 In/Out Message 用；奈秒精度，實務不碰撞）。
...
pub async fn run_autonomous(
    ...
    loop {
        ...
        // 每輪先服務一個 inbox task（訊息／事件 review 優先於承諾自主推進）。
        // 修復 doc/impl 不一致（原 doc 宣稱「每輪先清一個 inbox」但實作沒做）：承諾 session 期間
        // 到的訊息／事件 review，在下一輪循環開頭即被處理，不再苦等 session 結束 + 30s tick。
        if process_one_inbox_task(
            employee_id,
            &workspace_id,
            &mut memory,
    ...
    match &outcome {
        ...
        AutonomousOutcome::Stalled { reason, .. } => {
            let progress = if produced_any { "（已有部分產出）" } else { "" };
            memory
                .notes
...
mod tests {
    ...
    /// 對話回合（Phase 7b）：Human message task + Reasoner → Out Message（Ch.16）＋ artifact。
    #[tokio::test]
    async fn run_inbox_conversational_replies_to_message() {
        let dir = test_dir();
        let store = JsonStore::new(&dir);
        let emp_id = seed(&store);
        store
        ...
        // 一則 Out Message（員工回覆），內容來自 Reasoner。
        ...
        // task Completed、產一個 artifact（知識證據）。
    ...
    /// 卡住（規劃器不給 next_query）→ Stalled；承諾維持 Active（Phase 7b 起不自動 Suspended——
    /// 暫時性失敗不該凍結；下次喚醒重試）；員工睡回。
    #[tokio::test]
    async fn run_autonomous_stalls_keeps_active() {
        let dir = test_dir();
        let store = JsonStore::new(&dir);
        let emp_id = seed(&store);
        store
    ...
    /// D2 修復驗證：run_autonomous 每輪先清一個 inbox task——承諾 session 期間到的訊息／事件
    /// review 即時被處理，不再苦等 session 結束 + 30s tick。inbox task 優先於 plan/act/eval
    /// （消耗一個 budget cycle）。無此修復時 inbox task 會殘留 Assigned、無 Out Message、承諾 Stalled。
    #[tokio::test]
    async fn run_autonomous_processes_inbox_first() {
        let dir = test_dir();
        let store = JsonStore::new(&dir);
        let emp_id = seed(&store);
        store
            ...
            .unwrap();
        // 在 run_autonomous 啟動前就放一則訊息進 Inbox（模擬事件 review 投遞的 Assigned task）。
        store
            .put_task(&Task {
                id: "msg-1".into(),
                workspace_id: "ws".into(),
            ...
            .unwrap();
        let knowledge = StubTool::new("會議記錄：決議 A、待辦 B。");
        // reasoner 回應順序：① 對話回合（inbox review）→ answer；② PLAN → done；③ EVAL → done → Satisfied。
        let reasoner = StubReasoner::new(vec![
            r#"{"kind":"answer","text":"已審閱，決議 A 與待辦 B 已記下。"}"#,
            r#"{"done": true, "rationale": "已完成"}"#,
            r#"{"done": true, "rationale": "確認滿足"}"#,
        ]);
        let budget = CycleBudget {
            max_cycles: 5,
            max_duration: Duration::from_secs(10),
        ...
        // 核心斷言：inbox 訊息 task 已被處理（Completed）——session 期間即時消化（D2）。
        let task = store.get_task("msg-1").unwrap().unwrap();
        assert_eq!(task.status, TaskStatus::Completed);

        // 對話回合產出：一則 Out Message（員工回覆）。
        let msgs = store.list_messages_by_employee(&emp_id, 10).unwrap();
        assert!(
            msgs.iter().any(|m| m.direction == MessageDirection::Out),
            "應有員工回覆訊息（對話回合）"
        );

        // 承諾在 inbox 清完後仍正常推進至 Satisfied。
        assert!(matches!(outcome, AutonomousOutcome::Satisfied { .. }));
        assert_eq!(
            store.get_employee(&emp_id).unwrap().unwrap().state,
            EmployeeState::Sleeping
...
/// 殼層包裝）與排程器 `scan_commitments`（啟動喚醒）共用。已在忙則 no-op；保留「不每次 tick 燒 LLM」護欄。
pub async fn run_commitments_for_employee(
    state: &AppState,
    cfg: &app_config::AppConfig,
    db_path: &std::path::Path,
    employee_id: &str,
) -> anyhow::Result<()> {
    let _guard = match state.try_acquire(employee_id) {
        Some(g) => g,
        None => return Ok(()), // 該員工已在跑，跳過
    };
...
// ───────────────── 讀取面組裝（P2：殼 command 與 oserver handler 共用）─────────────────
//
// 介面只收 (&SqliteStore, &AppConfig)：不含任何 Tauri。`agent_os_enabled` 把關由
// 呼叫端（殼 command / oserver handler）負責——組裝函式本身不檢查。

/// 監看快照（`agent_watch`／`GET /api/employees/{id}/watch`）——state、Active
/// commitments、待辦 tasks、近期 artifacts、memory、近期 events、messages。
pub fn watch_payload(
    cfg: &app_config::AppConfig,
    store: &SqliteStore,
    employee_id: &str,
    ...
    Ok(serde_json::json!({
        ...
        "resolved_commitments": store
            ...
            .collect::<Vec<_>>(),
        "completed_tasks": store.list_tasks_by_owner(employee_id, &[TaskStatus::Completed, TaskStatus::Cancelled])?,
        "artifacts": store
...
/// 收件匣摘要（`agent_inbox_summary`／`GET /api/inbox`）：跨 workspace 的 Proposed
/// commitment（join 員工名）＋ Error/Paused 員工。
pub fn inbox_summary_payload(store: &SqliteStore) -> Result<InboxSummary, AppError> {
...
/// 近期活動流（`agent_recent_events`／`GET /api/events`）：跨員工事件＋員工名。
pub fn recent_events_payload(
    store: &SqliteStore,
    limit: usize,
